...
static esp_err_t http_server_setLimitsTempAndValuesRGB_json_handler(httpd_req_t *req)
{
	size_t header_len;
	char *header_value;
	char *row = NULL;
	...
	ESP_LOGI(TAG, "/setLimitsTempAndValuesRGB requested");
	...
	cJSON *value_Min_json = cJSON_GetObjectItem(root, "valueMin");
	cJSON *value_Max_json = cJSON_GetObjectItem(root, "valueMax");
	...
	if (row_json == NULL || value_Min_json == NULL || value_Max_json == NULL || value_R_json == NULL || value_G_json == NULL || value_B_json == NULL || !cJSON_IsString(value_Min_json) || !cJSON_IsString(value_Max_json) || !cJSON_IsString(value_R_json) || !cJSON_IsString(value_G_json) || !cJSON_IsString(value_B_json) || !cJSON_IsString(row_json))
	{
		cJSON_Delete(root);
		// Missing or invalid JSON fields
		// httpd_resp_send_err(req, HTTP_STATUS_BAD_REQUEST, "Missing or invalid JSON data fields");
	...
	valueMin = strdup(value_Min_json->valuestring);
	valueMax = strdup(value_Max_json->valuestring);
	...
	int value_Min = atoi(valueMin);
	int value_Max = atoi(valueMax);
	...
	// Now, you have the SSID and password in ssid_str and pass_str
	ESP_LOGI(TAG, "value_row: %d", Value_row);
	ESP_LOGI(TAG, "value_Min: %d", value_Min);
	ESP_LOGI(TAG, "value_Max: %d", value_Max);
	...
	char *status = setLimitTemp(Value_row, value_Min, value_Max, value_RED, value_GREEN, value_BLUE);
...
static esp_err_t http_server_show_temp(httpd_req_t *req)
{
	...
	xQueuePeek(Temperaturas, &value, pdMS_TO_TICKS(200));
...
static esp_err_t http_server_updateHour(httpd_req_t *req)
{
	...
	strftime(strftime_buf, sizeof(strftime_buf), "%Y", &timeinfo); // Convierte la hora actual a un formato de cadena legible por humanos
	int year = atoi(strftime_buf);

	if (year < 2000)
	{

		char *stringError = "Check internet connection - Error get hour";
		httpd_resp_set_type(req, "text/text");
		httpd_resp_send(req, stringError, strlen(stringError));
	}
	else
	{

		strftime(strftime_buf2, sizeof(strftime_buf2), "%c", &timeinfo);		  // Convierte la hora actual a un formato de cadena legible por humanos
		ESP_LOGI(tag, "The current date/time in Colombia is: %s", strftime_buf2); //

		httpd_resp_set_type(req, "text/text");
		httpd_resp_send(req, strftime_buf2, strlen(strftime_buf2));
	}
...
*/
		...
		httpd_uri_t set_RGB_json = {
			.uri = "/setLimitsTempAndValuesRGB",
			.method = HTTP_POST,
			.handler = http_server_setLimitsTempAndValuesRGB_json_handler,
			.user_ctx = NULL};
		...
		httpd_uri_t updateHour = {
			.uri = "/updateHour",
			.method = HTTP_GET,
			.handler = http_server_updateHour,
			.user_ctx = NULL};
		httpd_register_uri_handler(http_server_handle, &updateHour);
